...
data_cpi_filtered_combined = data_cpi_combined_steps[~data_cpi_combined_steps['INCWAGE'].isin(excluded_values)]

# Keep rows where 'AGE' is between 18 and 65
data_cpi_age_filtered = data_cpi_filtered_combined[(data_cpi_filtered_combined['AGE'] >= 18) & (data_cpi_filtered_combined['AGE'] <= 65)]
...
data_cpi_age_filtered['Male'] = data_cpi_age_filtered['SEX'].apply(lambda x: 1 if x == 'Male' else 0)
data_cpi_age_filtered['White'] = data_cpi_age_filtered['RACE'].apply(lambda x: 1 if x == 1 else 0)
data_cpi_age_filtered['Black'] = data_cpi_age_filtered['RACE'].apply(lambda x: 1 if x == 2 else 0)
data_cpi_age_filtered['Other_Races'] = data_cpi_age_filtered['RACE'].apply(lambda x: 1 if x == 3 else 0)
...
# Load the CSV file
data_cpi_combined_steps = pd.read_csv('/Users/liuliangcheng/Desktop/Duke/DE/usa_00004.csv')
...
mean_income_by_gender_race = data_cpi_age_filtered.groupby(['RACE', 'SEX'])['INCWAGE'].mean().round(2).unstack()
